...
class ZXView: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func addTimerNotification() {
        let timer = Timer.init(timeInterval: 2, target: self, selector: #selector(config), userInfo: nil, repeats: false)
