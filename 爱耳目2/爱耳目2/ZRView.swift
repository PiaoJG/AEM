...
class ZRView: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func addTimerNotification() {
        let timer = Timer.init(timeInterval: 1, target: self, selector: #selector(config), userInfo: nil, repeats: false)
